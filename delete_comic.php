<?php 
    ...
    if ($data == "xoa-truyen" && isset($_SESSION['user_id'])){
        ...
        EXECUTE($sql);
        $sql = "delete rd from readed rd join chapter chap on rd.id_chapter = chap.id join comic cm on chap.id_comic = cm.id where cm.id = ".$_POST['sid'];
        EXECUTE($sql);
        $sql = "delete pg from page pg join chapter chap on pg.id_chapter = chap.id join comic cm on chap.id_comic = cm.id where cm.id = ".$_POST['sid'];
        EXECUTE($sql);

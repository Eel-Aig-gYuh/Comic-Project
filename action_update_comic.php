<?php
    ...
    while(isset($_FILES['file-input']) && $_FILES['file-input']['size'] != 0) {
        ...
        $comic_name_fix = preg_replace("/[^A-Za-z0-9]/", "",  $comic_name);

        $target_file_new = $target_dir.str_replace(' ', '_', $comic_name_fix)."_".time().".".$imageFileType;
        ...
        if ($allowUpload) {
            ...
            if (move_uploaded_file($_FILES["file-input"]["tmp_name"], $target_file_new)) {
                ...
                EXECUTE($sql);
                $sql = "update comic set status='Đang tiến hành' where id = ".$comic_id;
                EXECUTE($sql);
    ...
    function them_trang_truyen($page, $chapter_id, $index_begin, $comic_name, $chapter_name) {
        ...
        for($index = 0; $index < $total; $index++) {
            ...
            // Kiểm tra nếu file đã tồn tại thì không cho phép ghi đè
            $comic_name_fix = preg_replace("/[^A-Za-z0-9]/", "",  $comic_name);
            $chapter_name_fix = preg_replace("/[^A-Za-z0-9]/", "",  $chapter_name);
            $target_file_new = $target_dir.str_replace(' ', '_', $comic_name_fix)."_".str_replace(' ', '_', $chapter_name_fix)."_".($index_begin+$index+1)."_".time();
    ...
    if(isset($_POST['chapter-updated']) && $_POST['chapter-updated'] != "") {
        ...
        foreach($arr_chap as $index_chap) {
            ...
            $sql = "update chapter set status='OK' where id = ".$chapter_id;
            EXECUTE($sql);

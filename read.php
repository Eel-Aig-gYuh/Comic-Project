...
<?php
    ...
    $chapter_t=strval((int)$_GET['chapter']+1);
    $chapter_tr=strval((int)$_GET['chapter'])-1;
...
<body>
                    ...
                    <div itemscope="itemscope" itemtype="http://schema.org/Brand" class="site-branding logox">
                        <h1 class="logo">
                            <a title="Trồng cà rốt" itemprop="url" href="./index.php"><img src="./img/image.png"
                                    alt="Trồng cà rốt" data-src="./img/image.png" decoding="async" class="lazyload"
                                    data-eio-rwidth="3000" data-eio-rheight="757"><noscript><img src='./img/image.png'
                                        alt='Trồng cà rốt' data-eio="l" /></noscript><span class="hdl">Trồng cà
                                    rốt</span></a>
                        </h1>
                        ...
                        <div itemscope="itemscope" itemtype="http://schema.org/Brand" class="site-branding logox">
                            <h1 class="logo">
                                <a title="Trồng cà rốt" itemprop="url" href="./index.php">
                                    <img src="./img/image.png" alt="Trồng cà rốt" data-src="./img/image.png"
                                        decoding="async" class=" ls-is-cached lazyloaded" data-eio-rwidth="3000"
                                        data-eio-rheight="757">
                                    <noscript>
                    ...
                    <ul id="menu-trang-chu" class="menu">
                        ...
                        <li id="menu-item-68"
                            class="menu-item menu-item-type-custom menu-item-object-custom menu-item-68"><a
                                href="https://www.facebook.com/profile.php?id=61553276407828&mibextid=ZbWKwL"><span
                                    itemprop="name">Facebook</span></a>
                        </li>
                    </ul>
                </nav>
            </div>
    ...
    <script type="text/javascript" src="./js/header.js" id="front-script-js"></script>
        ...
        <div class="contain_nav_breadvrumb">
            <nav class="nav_breadcrumb" aria-label="Page breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active">Chap <?php echo $_GET['chapter']." - ".$result[0]['name']; ?>
                    </li>
                </ol>
            </nav>
        </div>
                ...
                <div class="nvs">
                    <a href=<?php
                        if($chapter_tr<=0){
                            ...
                            echo '"./read.php?comic='.$comic_id.'&chapter='.$chapter_tr.'"' ;}
                            ?>>
                        <i class="fa fa-chevron-left" aria-hidden="true"></i> Prev Ch.</a>
                </div>
                <div class="nvs nvsc">
                    <div class="nvs slc">
                        <select class="select_paged" onchange="javascript:handleSelect(this)">
                            <?php
                        foreach ($chapter as $item) {
                            echo '<option value="./read.php?comic='.$comic_id.'&chapter='.$item['index'].'"';
                            if ($_GET['chapter']===$item['index']){
                                echo 'selected="true"';
                            }
                            echo' >Chương '.$item['index'].'</option>';
                        }
                        ?>
                        </select>
                    </div>
                </div>
                <div class="nvs rght"><a href=<?php
                        if(count($chapter)<$chapter_t){
                            echo '"#"';
                        }
                        else{

                            echo '"./read.php?comic='.$comic_id.'&chapter='.$chapter_t.'"' ;
                        }


                        ?>>Next Ch. <i class="fa fa-chevron-right" aria-hidden="true"></i></a></div>
            </div>
            <!-- <div class="nevxs right modlis">
                                    <div class="nvs slc">
                                        <select name="episode" id="slch"
                                            onchange="window.open(this.options[this.selectedIndex].value,'_self')">
                                            <option value="?read=list" selected="">List style </option>
                                            <option value="?read=paged">Paged style </option>
                                        </select>
                                    </div>
                                </div> -->
        </div>
    ...
    <div class="container-xxl" id="comment-area">
        ...
        <div class="comment comment-input" id="comment-0" style="display: inline-block;">
            ...
            <form id="user-comment-0" name="comment" method="POST" action="./action_postcomment.php">
                ...
                <input type="text" name="user_comic" value="<?php echo $result[0]['id_user']; ?>"
                    style="display: none;">
                <input type="text" name="link" value="./comic.php?comic=<?php echo $comic_id; ?>"
                    style="display: none;">
                <input type="text" name="account" value="<?php echo $user[0]['account_name']; ?>"
                    style="display: none;">
        ...
        function thump_up(id_cm, user, name, us_cm, cm) {
            ...
            if (like.classList.contains("bi-suit-heart")) {
                ...
                execute_query($query)
                $query = "insert into notification (id_user, type, content, link) values ('" + us_cm +
                    "', 'Thích bình luận', '" + name + " đã thích bình luận của bạn.', './comic.php?comic=" + cm + "')"
                execute_query($query)
            } else {

...
<?php
    ...
    $sql = "SELECT count(*) total from comic str where str.status = 'Đã hoàn thành' or str.status = 'Đang tiến hành' or str.status = 'Tạm ngưng' and str.updated_at >= '".$cenvertedTime."'";
    ...
    $sql = "SELECT str.id, str.name, str.coverphoto, str.total_chapter, str.detail, str.total_view, str.rating, str.status, str.id_user, str.author, str.updated_at, str.created_at, COUNT(fl.id_user) flow from comic str LEFT JOIN follow fl on str.id = fl.id_comic where str.status = 'Đã hoàn thành' or str.status = 'Đang tiến hành' or str.status = 'Tạm ngưng' and str.updated_at >= '".$cenvertedTime."' GROUP by str.id order by str.updated_at desc LIMIT $start, $limit";
        ...
        <div class="contain_nav_breadvrumb">
            <nav  class="nav_breadcrumb" aria-label="Page breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item">Truyện mới cập nhất</li>
                </ol>
            </nav>
        </div>
        <!--  -->

        <h2 class="caption">TRUYỆN MỚI CẬP NHẬT</h2>
        <div>

<?php
    ...
    if(isset($_SESSION['user_id'])) {
        ...
        $sql = "SELECT cm.id idcomic, cm.name, cm.coverphoto, cm.total_chapter, cm.detail, cm.total_view, cm.rating, cm.status, cm.id_user, cm.author, cm.created_at, cm.updated_at, fl.id,  fl.created_at FROM comic cm join follow fl on fl.id_comic = cm.id WHERE fl.id_user = ".$_SESSION['user_id']." GROUP by cm.id order by fl.created_at desc LIMIT $start, $limit";
